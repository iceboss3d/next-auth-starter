...
export default async function main(appName: string, dbUrl: string) {
  if (!appName) {
  ...
  await execa(
    "npx",
    ["shadcn@latest", "add", "form", "button", "input", "label", "card"],
    { stdio: "inherit" }
  );
...
`;
  await fs.outputFile("prisma/schema.prisma", schema);
...
`
  ...
  await fs.ensureDir("src/app/api/register");
  await fs.writeFile(
...
export async function POST(req: Request) {
  ...
  await fs.ensureDir("src/app/api/auth/[...nextauth]");
  await fs.writeFile(
...
export { handler as GET, handler as POST };
  ...
  await fs.ensureDir("src/app/auth/login");
  await fs.writeFile(
...
export default function LoginPage() {
  ...
  await fs.ensureDir("src/app/auth/register");
  await fs.writeFile(
